...
contract NFT is ERC721, Ownable {
  ...
  event PriceChange(address owner, uint price, uint id, string uri);
  ...
  function changePrice(uint _id, uint newPrice) public onlyOwner returns (bool){
    require(_exists(_id), "Error, wrong Token id"); //not exists

    //Update mapping
    price[_id] = newPrice;

    emit PriceChange(msg.sender, price[_id], _id, tokenURI(_id));
